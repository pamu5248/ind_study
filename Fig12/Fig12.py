...
df = pd.read_pickle('Fig12.pickle')
...
bounds_1 = .1
bounds_2 = .1
metric = 'd_veer_lidar_40_120'
countmean, total_binned_mean, ws_bins, bin_1_mean, bin_1_std, error_1, bin_2_mean, bin_2_std, error_2 = bin_by_metric(metric,'wind_speed_1_1',df,bounds_1,bounds_2,14)

fig, (ax1, ax2) = plt.subplots(2 , 1, sharex=True, figsize = (17,10))

n1 = len(df['active_power_1'][np.abs(df['d_veer_lidar_40_120'])>bounds_1])
n2 = len(df['active_power_1'][np.abs(df['d_veer_lidar_40_120'])<bounds_2])

ax1.fill_between(ws_bins, bin_1_mean-1*np.array(error_1), bin_1_mean+1*np.array(error_1),alpha = .25, color = 'C1')
ax1.plot(ws_bins,bin_1_mean,label = r'high |$\beta_{bulk}$| ' + '\n' + 'data available: ' + str(np.round(100*n1/(useable_data),1)) + '%', color = 'C1')

ax1.fill_between(ws_bins, bin_2_mean-1*np.array(error_2), bin_2_mean+1*np.array(error_2),alpha = .25, color = 'C0')
ax1.plot(ws_bins,bin_2_mean,label = r'low |$\beta_{bulk}$| ' + '\n' + 'data available: ' + str(np.round(100*n2/(useable_data),1)) + '%', color = 'C0')
