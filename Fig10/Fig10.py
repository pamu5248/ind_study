...
df = pd.read_pickle('Fig10.pickle')
...
bounds_1 = 0
bounds_2 = 0
countmean_nodir, total_binned_mean_nodir, ws_bins_nodir, bin_1_mean_nodir, bin_1_std_nodir, error_1_nodir, bin_2_mean_nodir, bin_2_std_nodir, error_2_nodir = bin_by_metric('rews_subtract_nacelle_speed_nodir','wind_speed_1_1',df,bounds_1,bounds_2)

fig, (ax1, ax2) = plt.subplots(2 , 1, sharex=True, figsize = (17,10))

n1 = len(df['active_power_1'][df['rews_subtract_nacelle_speed_nodir']>bounds_1])
n2 = len(df['active_power_1'][df['rews_subtract_nacelle_speed_nodir']<bounds_2])

ax1.fill_between(ws_bins, bin_1_mean-1*np.array(error_1), bin_1_mean+1*np.array(error_1),alpha = .25, color = 'C1')
ax1.plot(ws_bins,bin_1_mean,label = 'high REWS$_{NS}$ ' + '\n' + 'data available: ' + str(np.round(100*n1/(useable_data),1)) + '%', color = 'C1')

ax1.fill_between(ws_bins, bin_2_mean-1*np.array(error_2), bin_2_mean+1*np.array(error_2),alpha = .25, color = 'C0')
ax1.plot(ws_bins,bin_2_mean,label = 'low REWS$_{NS}$' + '\n' + 'data available: ' + str(np.round(100*n2/(useable_data),1)) + '%', color = 'C0')
...
ax2.fill_between(ws_bins, bin_1_mean-1*np.array(error_1)-total_binned_mean, bin_1_mean+1*np.array(error_1)-total_binned_mean,alpha = .25, color = 'C1')
ax2.plot(ws_bins,bin_1_mean-total_binned_mean,color = 'C1')

ax2.fill_between(ws_bins, bin_2_mean-1*np.array(error_2)-total_binned_mean, bin_2_mean+1*np.array(error_2)-total_binned_mean,alpha = .25, color = 'C0')
ax2.plot(ws_bins,bin_2_mean-total_binned_mean,color = 'C0')
